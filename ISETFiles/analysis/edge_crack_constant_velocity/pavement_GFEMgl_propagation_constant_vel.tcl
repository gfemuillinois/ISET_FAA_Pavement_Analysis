...
if { $argc != 4 } {
  puts "You must pass 4 arguments to this .tcl file!"
  exit
}

set env(VERTEX_TO_EXTRACT_SIF_ID) [ expr [ lindex $argv 0 ] ]

set xc [ expr [ lindex $argv 1 ] ]
set yc [ expr [ lindex $argv 2 ] ]
set zc [ expr [ lindex $argv 3 ] ]
...
#
# Setting high values next will enforce only PAE, which is fine for pavement simulations,
# and do not allow very strechted new facets apper.

crackMgr crackMgrID 1 setOptions frontStretchTol 110.0
crackMgr crackMgrID 1 setOptions accumulatedCrackFrontAdvanceLimit [ expr $num_steps * $delta_a_max_law ]
crackMgr crackMgrID 1 setOptions numStepsAccumDeltaA $num_steps
...
# crackMgr crackMgrID 1 crGrowthPhysicsLaw crFrontScalingLaw ConstFrontDispl
...
for { set i_step $starting_step } { $i_step < $num_steps } { incr i_step } {
  ...
  if { [ expr ( $i_step - $starting_step ) % 2 ] == 0 } {
    ...
    puts stdout "\n\n*********************************************"
    puts stdout "\nStarting Local Problem 100 Step $i_step"
    puts stdout "\n*********************************************\n\n"
  ...
  puts stdout "\n\n*********************************************"
  puts stdout "\nStarting Enriched Global Problem Step $i_step"
  puts stdout "\n*********************************************\n\n"
  ...
  if { $i_step == $starting_step } {
    ...
    for { set iter_gl 1 } { $iter_gl <= $max_iter_gl } { incr iter_gl } {
        ...
        puts stdout "\n\n*********************************************"
        puts stdout "\nStarting Enriched Global Problem Step $i_step G-L Iter $iter_gl"
        puts stdout "\n*********************************************\n\n"
  ...
  puts stdout "\n using DCM to extract SIF in global enriched problem"
