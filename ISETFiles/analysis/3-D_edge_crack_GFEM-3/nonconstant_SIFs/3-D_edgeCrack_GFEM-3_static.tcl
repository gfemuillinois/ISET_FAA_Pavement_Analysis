...
# New GFEM-3:
# This new strategy uses 3 scales to represent the crack.
#
# Parameters used to create the 3rd-scale local problems:
# Number of 3rd-scale local problems.
set nAuxLocalProbs 3
# Length of the overlapping region between two consecutive 3rd-scale local problems.
set overlap 0.35
# Crack front length.
set crackFrontLength 3.0
# Minimum x, y, or z coordinate among all the vertices that are on the crack front.
# For instance, if the crack front is aligned with the z direction, as is the case of this example,
# this is the minimum z coordinate among all the crack front vertices. OTOH, if the crack
# front is aligned with the x or y direction, this is the minimum x or y coordinate, respectively, among all
# the crack front vertices.
set crackFrontFirstCoord -1.5
...
crackMgr crackMgrID 1 crGrowthPhysicsLaw crFrontScalingLaw setConstFrontDispl false
...
# The following option is used for SIMPLIFIED simulations only:
# If false, SIFs will not be constant along crack front.
crackMgr localProb 10 crackMgrID 11 crGrowthPhysicsLaw crFrontScalingLaw setConstFrontDispl false
...
# This is used to compute DCM parameters when extracting SIFs:
# It will be the minimum edge length among all 3rd-scale local problems.
set minEdgeLenDCM $minEdgeLen

# New GFEM-3:
# Defining third-scale local problems:
for { set auxLocalProb 0 } { $auxLocalProb < $nAuxLocalProbs } { incr auxLocalProb } {
    
  # ************************** Create 3rd-scale local problems 100, 200, ... **************************
  set localProbId [ expr 100 * ( ${auxLocalProb} + 1 ) ]
  ...
  # creating the 3rd-scale LP 100, 200, ...
  ...
  set z_min_auxlp [ expr $crackFrontFirstCoord + ( $crackFrontLength / $nAuxLocalProbs ) *   $auxLocalProb       - $overlap ]
  set z_max_auxlp [ expr $crackFrontFirstCoord + ( $crackFrontLength / $nAuxLocalProbs ) * ( $auxLocalProb + 1 ) + $overlap ]
  ...
  set crackMgrId [ expr 111 * ( ${auxLocalProb} + 1 ) ]
  ...
  if { $minEdgeLenDCM > $minEdgeLen } {

    set minEdgeLenDCM $minEdgeLen
    puts stdout "\n ** Updated minEdgeLenDCM = $minEdgeLenDCM"
  }
  ...
  # The following option is used for SIMPLIFIED simulations only:
  # If false, SIFs will not be constant along crack front.
  crackMgr localProb $localProbId crackMgrID $crackMgrId crGrowthPhysicsLaw crFrontScalingLaw setConstFrontDispl false

  if { $solveLocalProbsInParallel == false } {
...
if { $solveLocalProbsInParallel == true } {

  # Assemble
  set initTime [ clock clicks -milliseconds ]
  assemble allLocalProbs
  set finalTime [ clock clicks -milliseconds ]
  set totalTime_pa [ expr $totalTime_pa + ( $finalTime - $initTime ) / 1.0e+03 ]
  puts stdout "\nTime spent for assemble command: $totalTime_pa s \n"
  ...
  # Generate post-process VTK of local mesh and results:
  for { set auxLocalProb 0 } { $auxLocalProb < $nAuxLocalProbs } { incr auxLocalProb } {
    set localProbId [ expr 100 * (${auxLocalProb} + 1) ]
...
set dist [ expr 4.0 * $minEdgeLenDCM ]
set deltaD [ expr 0.1 * $minEdgeLenDCM ]
set num 11
...
if { $isVisco == true } {
  ...
  for { set auxLocalProb 0 } { $auxLocalProb < $nAuxLocalProbs } { incr auxLocalProb } {
    ...
    # Associated crack manager ID:
    set crackMgrId  [ expr 111 * ( ${auxLocalProb} + 1 ) ]

    crackMgr localProb $localProbId crackMgrID $crackMgrId extract DCM \
                                                           extrDist_min $dist \
                                                           deltaDist $deltaD \
                                                           numExtrPoints $num \
                                                           start_time $s_time \
