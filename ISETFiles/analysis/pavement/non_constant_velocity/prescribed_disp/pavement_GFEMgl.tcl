# ************************** pavement_GFEMgl_fully3d.tcl **************************

set crackFile "Fully_Crack"
set resultsFolder "res"
...
# This value (1.0) will lead to crack arrest before running all simulations steps.
# set myGc 1.0
...
crackMgr crackMgrID 1 setOptions useMLSForCrackFrontVertices true

# Set to ignore KII since we know there is no Mode II:
# crackMgr crackMgrID 1 setOptions ignoreKII true

# Set to ignore KIII since we know there is no twisting in this example:
...
if { $isVisco == true } {
  ...
  # If viscoFourierTransf false: use Zakian's method (NOT recommended).
...
for { set i_step $starting_step } { $i_step < $num_steps } { incr i_step } {
  ...
  if { $isVisco == true } {
    ...
    # Recommended number to time steps
